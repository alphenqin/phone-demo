...
<head>
    ...
    <script>
        ...
        $(function (){
            ...
            <#if !loginMember ??>
                //*[属性名],#id,
                $("*[data-read-state],#btnEvaluation,*[date-evaluation-id]").click(function (){
                    // 未登录情况下，点击想看/看过，点赞，短评功能，提示登录页面
            ...
            <#if loginMember ??>
                ...
                $("#btnSubmit").click(function (){
                    ...
                    //通过ajax将内容通过uri传给controller
                    $.post("/evaluate",{
                        score: score,
                        bookId: ${book.bookId},
                        memberId: ${loginMember.memberId},
                        content: content
                        //经过controller处理后的返回的数据通过json返回页面
                    },function (json){
                        if(json.code == "0"){
                            //刷新当前界面
